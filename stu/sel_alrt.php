...
if($result)
{
	$pqr="UPDATE student_login SET alert_time=NOW() WHERE roll_number='$username'";
	$res=mysql_query($pqr);
	...
	while($row=mysql_fetch_array($result))
	{
		if($row['alert_to_type']==6)
		{
				...
				echo "<tr>";
				echo "<td>Alert From ".getName1($row['alert_by'])."</td>";
				echo "<td>".$row['alert_time']."</td>";
				echo "<td>"."<a href=\"view_alrt2.php?id=$id\">View Alert</a>"."</td>";
				echo "</tr>";
				$sid++;
		}
		elseif($row['alert_to_type']==7&&$row['alert_to']==$branch)
		{
				$id=mcrypt($row['uni_id']);
				echo "<tr>";
				echo "<td>Alert From ".getName1($row['alert_by'])."</td>";
...
{
	echo "<h2><center>Some error encontered</center></h2>";
}
