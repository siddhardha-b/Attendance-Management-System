    ...
    <ul id="menu" style="margin-top: 0px; vertical-align: middle;">
			...
			echo "<li  onmouseover=\"menu_hold(this)\" onmouseout=\"menu_out(this)\">
                <a href=\"#\" class=\"drop\"><span>View Passwords</span></a>
                       ...
                       <li class="menu_right" style="color: #CCC;"><?php session_start(); if($_SESSION['user_level']!=4) echo "<a href=\"contactadmin.php\">Contact admin</a>"; session_write_close();?></li> 
    </ul>
    </div>
		<div class="maincontent">
						<div class="evaluationcontent-support">
			<span style="color: #B22222; font-weight: bold; font-size: 12px;">
				Welcome<?php session_start(); echo " ".$_SESSION['NAME']; session_write_close();?>		</span>
		</div>
...
$faculty_id=$_GET['thisfac_id'];
$per_id=$_GET['per_id'];
$ydm=date('Y-m-d');
$query="SELECT * FROM faculty_assign_class WHERE to_faculty='$faculty_id' AND ydm='$ydm'";
...
if($result)
{
	if(mysql_num_rows($result)>0)
	{
		...
		{
			if($row['per_num']==$per_id)
			{
				$query="DELETE FROM faculty_assign_class WHERE per_num ='$per_id'";
				$result=mysql_query($query);
				if($result)
				{
					echo "Deleted Successfully.Redirecting you.";
					header("refresh:2;url=view_fac.php?id=$faculty_id");
					break;
				}
				else
				{
					echo "error in deleteing.Redirecting You.";
					header("refresh:2;url=view_fac.php?id=$faculty_id");
				}
			}
		}
	...
	{
		echo "No such Assignmnet exists currently.redirecting you.";
		header("refresh:2;url=view_fac.php?id=$faculty_id");
	}
}
else
{
	echo "error in querying.redirecting you.";
	header("refresh:2;url=view_fac.php?id=$faculty_id");
}
?>
