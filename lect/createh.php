...
$sfrm=mysql_real_escape_string($_POST['sname']);
$dep=mysql_real_escape_string($_POST['dep']);
$email=mysql_real_escape_string($_POST['email']);
$password=randomPassword();
$password=ABEncryption($id,$password);
$dep=strtoupper($dep);
	if(!empty($id))
	{
		$qer1="SELECT * FROM faculty_login WHERE user_name=$id";
		$res=mysql_query($qer1);
		if(mysql_num_rows($res)>0)
		{
			$quer=mysql_query("UPDATE faculty_login SET user_level='2' WHERE user_name=$id") or die(mysql_error());
			if($quer)
			{
				echo "<h3><center>Sucesfully created</center></h3>";
			}
		}
		else{
		if(!empty($name)&&!empty($lname)&&!empty($id)&&!empty($email)&&!empty($dep))
		{
			$query=mysql_query("INSERT INTO faculty_login (user_name,password,name,last_name,faculty_email,faculty_dept,user_level) VALUES('$id','$password','$name','$lname','$email','$dep','2')") or die(mysql_error());
			$query2=mysql_query("INSERT INTO faculty_details (user_name,faculty_dept,faculty_name,faculty_sfrm,faculty_last_name) VALUES ('$id','$dep','$name','$sfrm','$lname')" ) or die(mysql_error());
			$query3=mysql_query("INSERT INTO faculty_schedule_yearsecsub (user_name,faculty_dept) VALUES ('$id','$dep')")or die(mysql_error());
	...
	{
		echo "<script type=\"text/javascript\">";
		echo "alert(\"Please enter faculty id.\")";
		echo "</script>";
	}
...
<tr><td>Id No.</td><td><input type="text" name="id" \></td></tr>
<tr><td>Short Name</td><td><input type="text" name="sname" /></td></tr>
<tr><td>Dept</td><td><input type="text" name="dep" /></td></tr>
...
<a href="delh.php"><h2><center>Click here delete HOD</center></h2></a>
        </table>
</div></p>
								</div>
							</div>
