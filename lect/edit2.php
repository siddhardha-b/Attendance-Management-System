...
$qur="SELECT * FROM student_current_details WHERE roll_number=$roll LIMIT 1";
$res=mysql_query($qur) or die(mysql_error());
echo"<table class=\"inner_table1\" border=\"1\">";
if($row=mysql_fetch_array($res))
{
$roll=$row['roll_number'];
$query="SELECT * FROM student_login WHERE roll_number=$roll LIMIT 1";
$result=mysql_query($query);
if($row1=mysql_fetch_array($result))
{$name=$row1['name'];
$email=$row1['student_email'];
$stat=$row1['user_status'];
echo "<tr><td>roll</td><td>$roll</td></tr><tr><td>Name</td><td><input type=\text\" name=\"sname\" value=$name></td></tr><tr><td>Email</td><td><input type=\text\" name=\"email\" value=$email></td><tr><td>user Status</td><td><select name=\"stat\" value=$stat><option value=\"1\">1</option><option value=\"0\">0</option></select></td></tr>";
}
echo "Basic Details";

$query1="SELECT * FROM student_basic WHERE roll_number=$roll LIMIT 1";
$result1=mysql_query($query1);
if($row2=mysql_fetch_array($result1))
{
$jyear=$row2['joining_year'];
$jbranch=$row2['join_branch'];
$sm=$row2['stu_mobile'];
$pm=$row2['parent_mobile'];
echo"<tr><td>join year</td><td><input type=\"text\" name=\"jyear\" value=$jyear /></td></tr><tr><td>join branch</td><td><input type=\"text\" name=\"jbranch\" value=$jbranch ></td></tr>";
echo"<tr><td>student mobile</td><td><input type=\"text\" name=\"sm\" value=$sm /></td></tr><tr><td>parent mobile</td><td><input type=\"text\" name=\"pm\" value=$pm /></td></tr>";
}
$csec=$row['current_sec'];
$cyear=$row['current_year'];
$csem=$row['current_sem'];
$cbranch=$row['current_branch'];
echo"<tr><td>Current Section</td><td><input type=\"text\" name=\"csec\" value=$csec></td></tr><tr><td>Current year</td><td><input type=\"text\" name=\"cyear\" value=$cyear></td></tr>";
echo"<tr><td>Current Semester</td><td><input type=\"text\" name=\"csem\" value=$csem></td></tr><tr><td>Current branch</td><td><input type=\"text\" name=\"cbranch\" value=$cbranch></td></tr>";
}
echo"</table>";
...
$jyear=mysql_real_escape_string($_POST['jyear']);
$jbranch=mysql_real_escape_string($_POST['jbranch']);
$csec=mysql_real_escape_string($_POST['csec']);
$cyear=mysql_real_escape_string($_POST['cyear']);
$csem=mysql_real_escape_string($_POST['csem']);
$cbranch=mysql_real_escape_string($_POST['cbranch']);
$sm=mysql_real_escape_string($_POST['sm']);
$pm=mysql_real_escape_string($_POST['pm']);

$quert=mysql_query("UPDATE student_login SET name='$name',student_email='$email',user_status='$stat' WHERE roll_number='$roll'") or die(mysql_error());
$quert1=mysql_query("UPDATE student_basic SET joining_year='$jyear',join_branch='$jbranch',stu_mobile='$sm',parent_mobile='$pm' WHERE roll_number='$roll'") or dile(mysql_error());
$quert2=mysql_query("UPDATE student_current_details SET current_year='$cyear',current_branch='$cbranch',current_sec='$csec',current_sem='$csem' WHERE roll_number='$roll'") or dile(mysql_error());


if($quert&&$quert1&&$quert2)
{
