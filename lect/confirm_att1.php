...
if(!isset($_GET['year'])||!isset($_GET['sec'])||!isset($_GET['sp'])||!isset($_GET['np'])||!isset($_GET['no_stu'])						||!isset($_GET['no_pre'])||!isset($_GET['subject'])||!isset($_GET['no_abs'])||!isset($_GET['date']))
{
...
$no_stu=$_GET['no_stu'];
$no_pre=$_GET['no_pre'];
$subject=mysql_real_escape_string(strtoupper($_GET['subject']));
$p=unserialize($_GET['pre']);
$no_abs=$_GET['no_abs'];
$a=unserialize($_GET['abs']);
echo "<h1><center>presentees are</center></h1>.<br/>";
$x=0;

if(!(isAlreadyEntered($sp,$year,$section)&&!isLiveSem($year)))
{echo "<center>";
	foreach($p as $i)
	{
		$x++;
		echo substr($i,-3)."&nbsp&nbsp&nbsp&nbsp&nbsp";
		if($x%5==0)
		{
			echo "<br/>";
		}
	}
	echo "<h1><center>Absentees are</center></h1>.<br/>";
	$x=0;
	echo "<center>";
	foreach($a as $i)
	{
	...
	if(isset($_POST['submit']))
	{
		...
		$trans_flag=true;
		$sub_taught=mysql_real_escape_string($_POST['sub_tau']);
		if(empty($sub_taught))
		{
			$trans_flag=false;
			echo "<script type=\"text/javascript\">";
			echo "alert(\"you need to enter topic taught\")";
			echo "</script>";
			exit();
		}
		for($x=0;$x<$np;$x++)
		{
			...
			for($i=0;$i<$no_pre;$i++)
			{
				$pre_id=generatePresentee1($date);
				$query="INSERT INTO att_table (roll_number,date,year,section,period,subject,p_or_a) VALUES ('$p[$i]','$dmy','$year','$section','$period','$subject','$pre_id')";
				$result=mysql_query($query);
				if(!$result)
				{
					//die("Error querying.Too many connections.exiting now.");
					$trans_flag=false;break;
					//header("refresh:5;url=view_tt.php");
				}
			}
			for($j=0;$j<$no_abs;$j++)
			{
				$abs_id=generateAbsentee1($date);
				$query="INSERT INTO att_table (roll_number,date,year,section,period,subject,p_or_a) VALUES ('$a[$j]','$dmy','$year','$section','$period','$subject','$abs_id')";
				$result=mysql_query($query);
				if(!$result)
				{
					//die("Error querying.Too many connections.exiting now.");
		...
		if($trans_flag)
		{
			for($x=0;$x<$np;$x++)
			{
				$query="INSERT INTO comp_class_details (user_name,date,year,section,period,subject,sub_taught) VALUES('$username','$dmy','$year','$section','$spp','$subject','$sub_taught')";
				$result=mysql_query($query);
				if(!$result)
				{
					//echo "not entered";
		...
		{
			mysql_query("COMMIT");
			header("location:id.php?id=1");
		}
		elseif(!$trans_flag)
		{
			mysql_query("ROLLBACK");
			header("location:id.php?id=0");
		
		}
	}
	elseif(isset($_POST['cancel']))
	{
		echo "<p align=top>Please wait.we are redirecting you.</p>";
		header("refresh:1;url=enter_att_wnou.php");
	}
}
...
<center><form id="form1" name="form1" method="post" action="">
  <table width="500" border="0">
    <tr>
      <td>Enter topic taught</td><br>
      <td><label for="sub_tau"></label>
      <textarea name="sub_tau" id="sub_tau" cols="45" rows="5"></textarea></td>
    </tr>
    </table>
