...
if(isset($_POST['submit']))
{
	...
	if(empty($a))
	{
		echo "<center><h2>Subject ID cannot be empty</h2></center>";
	}
	else
	{
		if(isElective($a))
		{
			$query="DELETE FROM subject_details WHERE subject_id='$a';DELETE FROM sub_electives WHERE ele_id='$a'";
		}
		else
		{
			$query="DELETE FROM subject_details WHERE subject_id='$a';";
		}
		if(mysql_query($query))
		{
			echo "<h2><center>Sucessfully Deleted Subject</center></h2>";
		}
		else
		{
			echo "<h2><center>An unknown error encontered while entering details.Try later</center></h2>";
		}
...
<form id="form1" name="form1" method="post" action="">
  <table width="500" border="0" class="inner_table1">
    <tr>
      ...
      <td><label for="subid"></label>
      <input type="text" name="subid" id="subid" /></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
