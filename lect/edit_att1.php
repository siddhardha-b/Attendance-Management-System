...
if(!isset($_GET['date'])||!isset($_GET['year'])||!isset($_GET['section'])||!isset($_GET['per_id']))
{
	exit();
}
?>
...
<?php
	...
	if($result)
	{
		...
		{
			$i=0;$x=0;
				echo "<center><form action=\"testedit.php\" id=\"form1\">";
				echo "<input type=\"radio\" name=\"year\" id=\"year\" value=\"$year\" checked style=\"display:none\">";
				echo "<input type=\"radio\" name=\"section\" id=\"section\" value=\"$section\" checked style=\"display:none\">";
				echo "<input type=\"radio\" name=\"period\" id=\"period\" value=\"$sp\" checked style=\"display:none\">";
				echo "<input type=\"radio\" name=\"date\" id=\"period\" value=\"$date\" checked style=\"display:none\">";
				echo "<br/>";
			while($row=mysql_fetch_array($result))
			{
				if($row['section']==$section&&$row['period']==$sp)
				{
					$r[$i]=$row['uni_id'];
					$val=decrypt1($row['p_or_a'],$date);
					echo "<b>".substr($row['roll_number'],-3)."</b> <input type=\"text\" name=\"$r[$i]\" value=\"$val\" style=\"width:40px;\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					$i++;
					if($i%5==0)
					{
						echo "<br/>";
			...
			if($i==0)
			{
				...
				echo "</script>";
				echo "<h1><center>No such entries found</center></h1>";
				header("refresh:2;url=edit_att.php");
				exit();
			}
		}
		else
		{
			echo "<h2><center>No such entry found.</center></h2>";
		}
	}
	else
	{
