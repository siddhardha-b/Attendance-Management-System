...
if(!isset($_GET['id'])||!isset($_GET['year'])||!isset($_GET['sem']))
{
	exit();
}
?>
...
if($id==0)
{
	...
	elseif($sem==2)
	{
		...
		$query="SELECT * FROM sem_table WHERE branch='$fac_branch' AND year='$year'";
		...
		if($result)
		{
			if(mysql_num_rows($result)>0)
			{
				while($row=mysql_fetch_array($result))
				{
					if($row['sem']==$nsem)
					{
						if($row['status']==1)
						{
							header("location:nsem.php?errid=0&nsem=$nsem");
						}
					}
				}
			}
			...
			{
				header("location:nsem.php?errid=1");
			}
		}
		else
		{
			header("location:nsem.php?errid=2");
		}
		$query="SELECT * FROM sem_table WHERE branch='$fac_branch' AND year='$year'";
		$result=mysql_query($query);
		if($result)
		{
			if(mysql_num_rows($result)>0)
			{
				while($row=mysql_fetch_array($result))
				{
					...
					{   $date=date("Y-m-d");
						$uni=$row['uni_id'];
						$query2="UPDATE sem_table SET status='1',start_date='$date' WHERE uni_id='$uni'";
						$result2=mysql_query($query2);
						if($result2)
						{
							header("location:nsem.php?sucid=1");
						}
						else
						{
							header("location:nsem.php?errid=3");
						}
					}
				}
			...
			{
				header("location:nsem.php?errid=4");
			}
		}
		else
		{
			header("location:nsem.php?errid=5");
		}
		...
		if($result)
		{
			if(mysql_num_rows($result)>0)
			{
				while($row=mysql_fetch_array($result))
				{
					...
					{
						$uni=$row['uni_id'];
						$query2="UPDATE sem_table SET status='0' WHERE uni_id='$uni'";
						$result2=mysql_query($query2);
						if($result2)
						{
							header("location:nsem.php?sucid=2");
						}
						else
						{
							header("location:nsem.php?errid=3");
