...
if ($data[0]) { 
	 $roll=$data[1];
	 $email=substr($roll,-8);
	 $email=$email."@mvsrec.edu.in";
	 $branch=strtoupper($data[2]);
           $quer=mysql_query("INSERT INTO student_current_details (roll_number,current_branch,current_sec,current_year,current_sem) VALUES ('$data[1]','$branch','$data[4]','$data[5]','1')") or die(mysql_error());
			
			$quer1=mysql_query("INSERT INTO student_basic (roll_number,join_branch,joining_year,stu_mobile,parent_mobile) VALUES ('$data[1]','$branch','0','$data[7]','$data[8]')") or die(mysql_error());
			$quer2=mysql_query("INSERT INTO admissiondetails(roll_no,student_email,student_mobile) VALUES ('$data[1]','$email','$data[7]')") or die(mysql_error());
			$password=randomPassword();
			
			$roll2="p_".$roll;
			$password=ABEncryption(substr($roll,-9),$password);
			$quer3=mysql_query("INSERT INTO student_login (roll_number,name,password,parent_name,student_email) VALUES ('$data[1]','$data[3]','$password','$data[6]','$email')") or die(mysql_error());
		
			$password1=randomPassword();
			$password1=ABEncryption(substr($roll,-9),$password1);
			
			$quer4=mysql_query("INSERT INTO student_login (roll_number,name,password,parent_name,user_level,student_email) VALUES ('$roll2','$data[3]','$password1','$data[6]','12','$email')") or die(mysql_error());
		if((!$quer)&&(!$quer1)&&(!$quer2)&&(!$quer3)&&(!$quer4))
		{
		$trans_flag=false;
break;
		}
...
{
	mysql_query('COMMIT');
	echo"<h2><center>Succesfully created student details.</center></h2>";
}
else 
{
	mysql_query("ROLLBACK");
	echo "<h3><center>Error in creating student details.Please check the corresponding CSV file for any errors.</center></h3>";
}
} 
else
{ 
		echo "<script type=\"text/javascript\">";
		echo "alert(\"Please upload CSV file only.\")";
		echo "</script>";
}
}
...
<br>
<br>
<center>Format: serial number, roll number, branch, name, section, current year, parent name, parent mobile, student mobile, joining year</center> <br>
</br>
<center>
Example: 1, 245112733177, Alice, CSE3, 1, Bob, 9xxxxxxxxx, 9xxxxxxxxx, 2012
</center>
</h3>
<br>
<br>
<center><form action="" method="post" enctype="multipart/form-data" name="form1" id="form1"> 
  Choose your CSV file: <br /> <br/>
