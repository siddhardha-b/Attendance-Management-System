...
echo "<h2><center>Altough you delete this alert, a copy of this alert exists in database.";
if(isset($_POST['cancel']))
{
	header("location:dlt_alrt.php");
}
elseif(isset($_POST['submit']))
{
	...
	if($result)
	{
		...
		$alert_by=$row['alert_by'];
		$time_created=$row['alert_time'];
		$alert_to_type=$row['alert_to_type'];
		$alert_to=$row['alert_to'];
		$alert_subject=$row['alert_subject'];
		$alert_details=$row['alert_details'];
		$query1="INSERT INTO deleted_alerts (alert_time,alert_by,alert_to_type,alert_to,alert_subject,alert_details) VALUES('$time_created','$alert_by','$alert_to_type','$alert_to','$alert_subject','$alert_details')";
		$result1=mysql_query($query1);
		if($result1)
		{
			$query2="DELETE FROM alert_section WHERE uni_id='$id1'";
			$result2=mysql_query($query2);
			if($result2)
			{
				header("location:dlt.php?id=0");
			}
			else
			{
				header("location:dlt.php?id=1");
				exit();
			}
		}
		else
