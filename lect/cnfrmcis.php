...
if($_GET['flg']==0)
{
	echo "<h1><center>Faculty is free on the selected period</center></h1>";
	...
	{
		$ydm= date('Y-m-d');
		$frm_faculty=$_GET['dis_fac'];
		$to_faculty=$_GET['fac_id'];
		$per_num=$_GET['per_num'];
		$per_details="FREE";
		$upd=addComma($_GET['dis']);
		if(!isAlreadyAssigned($to_faculty,$per_num,$ydm))
		{
			$query="INSERT INTO faculty_assign_class (user_name,ydm,to_faculty,per_num,per_details,user_per_details)														VALUES('$frm_faculty','$ydm','$to_faculty','$per_num','$per_details','$upd')";
			$result=mysql_query($query);
			if($result)
			{
				header("location:cnf.php?id=0");
			}
			else
			{
				header("location:cnf.php?id=1");
			}
		}
		else
		{
			header("location:cnf.php?id=2");
		}
	}
}
elseif($_GET['flg']==1)
{
	...
	$subject=getPeriod($_GET['subject']);
	echo "<h1><center>Faculty has the follwing class.</center></h1><br/>";
	echo "<h1><center>".$year."<br/>".$section."<br/>".$subject."</center></h1>";
	if(isset($_POST['cancel']))
	{
		header("location:assign_class.php");
	}
	if(isset($_POST['confirm']))
	{
		...
		$to_faculty=$_GET['fac_id'];
		$per_num=$_GET['per_num'];
		$per_details=$year.",".$section.",".$subject;
		$upd=addComma($_GET['dis']);
		if(!isAlreadyAssigned($to_faculty,$per_num,$ydm))
		{
			$query="INSERT INTO faculty_assign_class (user_name,ydm,to_faculty,per_num,per_details,user_per_details)														VALUES('$frm_faculty','$ydm','$to_faculty','$per_num','$per_details','$upd')";
