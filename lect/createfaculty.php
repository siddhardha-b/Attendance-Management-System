...
if(empty($data[0])||empty($data[1])||empty($data[2])||empty($data[3])||empty($data[4]))
      {$trans_flag=false;break;} 
          if ($data[0]) { 
            $quer=mysql_query("INSERT INTO faculty_details (user_name,faculty_dept,faculty_sfrm,faculty_name,faculty_last_name) VALUES 
                ( 
			...
			$quer1=mysql_query("INSERT INTO faculty_schedule_yearsecsub (user_name,faculty_dept) VALUES ('$data[1]','$dep')") or die(mysql_error());
			$password=randomPassword();
			$roll=$data[1];
			$password=ABEncryption($roll,$password);
			$mail=$data[1]."@mvsrec.edu.in";
			$quer2=mysql_query("INSERT INTO faculty_login (user_name,name,last_name,password,faculty_email,faculty_dept) VALUES ('$data[1]','$data[3]','$data[4]','$password','$mail','$dep')")or die(mysql_error());
...
if($trans_flag)
{
	echo"<h2><center>sucessfully created</center></h2>";
	mysql_query("COMMIT");
...
else 
{
		...
		echo "<h3><center>not a succesfull transaction.try again later.we are redirecting you</center></h3>";
			header("refresh:5;home.php");
} 
}
else echo"uploaded file is not a CSV file please make sure and upload CSV file";
} 
}
catch(Exception $e)
{
...
<h3><center>
<br><br>
Format: serial number, faculty id, faculty name short notation, first name, last name.
<br><br><br>
Example: 46, 111111941, GMR, GAUTAM MURTHY, K R.</center>
</h3><br><br>
...
<center><a href="entryform.php"><h3><center>Click here to enter details of single faculty</center></h3></a> 
   </table>
</div></p>
								</div>
							</div>
