...
$dep=$_SESSION['dept'];
$qur="SELECT * FROM faculty_details WHERE user_name=$roll";
...
{
$roll=$row['user_name'];
$dep=$row['faculty_dept'];
$name=$row['faculty_name'];
$lastname=$row['faculty_last_name'];
$sfrm=$row['faculty_sfrm'];
echo"<tr><td>faculty id</td><td>$roll</td><tr><td>Department</td><td><input type=\"text\" name=\"dep\" value=$dep></td></tr><tr><td>Name</td><td><input type=\"text\" name=\"name\" value=$name></td></tr>
<tr><td>Lastname</td><td><input type=\"text\" name=\"lname\" value=$lastname></td></tr><tr><td>Short Form</td><td><input type=\"text\" name=\"sfrm\" value=$sfrm></td></tr>";


$query="SELECT * FROM faculty_login WHERE user_name=$roll LIMIT 1";
$result=mysql_query($query);
if($row1=mysql_fetch_array($result))
{
$email=$row1['faculty_email'];
$stat=$row1['user_status'];
echo"<tr><td>Email</td><td><input type=\"text\" name=\"email\" value=$email></td></tr><tr><td>User Status</td><td><input type=\"text\" name=\"stat\" value=$stat></td></tr>";
}
}
...
$sfrm=mysql_real_escape_string($_POST['sfrm']);
...
$query=mysql_query("UPDATE faculty_login SET name='$name',last_name='$lname',faculty_email='$email',faculty_dept='$dep',user_status='$stat' WHERE user_name='$roll'") or die(mysql_error());
$query2=mysql_query("UPDATE faculty_details SET faculty_name='$name',faculty_last_name='$lname',faculty_sfrm='$sfrm',faculty_dept='$dep' WHERE user_name='$roll'") or die(mysql_error());
$query3=mysql_query("UPDATE faculty_schedule_yearsecsub SET faculty_dept='$dep' WHERE user_name='$roll'") or die(mysql_error());
