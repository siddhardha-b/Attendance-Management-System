...
<?php
$fac_id=$_SESSION['SESS_FIRST_NAME'];
...
if(isset($_POST['submit']))
{
	...
	if(mysql_num_rows($r)==0&&!empty($section))
	{
			...
			echo "alert(\"Invalid data provided\")";
			echo "</script>";
			echo "<h2><center>You entered invalid data</center></h2>";
	}
	if(!empty($year)&&!empty($section)&&mysql_num_rows($r)>0)
	{
		...
		{
			$row=mysql_fetch_assoc($result);
			if($row['current_branch']!=$branch)
			{
				echo "<h2><center>Please enter a section of your branch</center></h2>";
			}
			else
			{
				header("location:elective_enter.php?year=$year&sec_id=$section");
				echo "entered";exit();
			}	
		}
		else
		{
			echo "please choose a valid section";
		}
	}
	if(empty($section))
	{
			...
			echo "</script>";
			echo "<h2><center>Section cannot be empty</center></h2>";
	}
