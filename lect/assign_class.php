<?php
	...
	require_once('../funcs/checkday.php');
...
$period=$_GET['per_id'];
//echo "you are assigning class of".getName($GET['thisfac_id'])."on".$_GET['per_id']."period";
$det=array();
$det1="";
$errflag=false;
	if(isset($_GET['fac_id']))
	{
		...
		if(!empty($fac_id)&&!empty($period))
		{
			if(!validUsername($fac_id))
			{
				$errflag=true;
				$msg[]="Invalid faculty id.<br/>";
			}
			if(!$errflag)
			{
				$day1=date('l');
				$det=getPeriodByDay($day1,$period,$fac_id);//need to be changed
				$det1=implode(getPeriodByDay($day1,$period,$this_faculty));
				$year1=$det[0];
				if(!($year1=="FREE")&&!($year1==NULL))
				{
					$section1=$det[1];
					$subject1=$det[2];
				}
				...
				{
					$flg=0;
					header("location:cnfrmcis.php?flg=$flg&fac_id=$fac_id&per_num=$period&dis=$det1&dis_fac=$this_faculty");
				}
				else
				{
					$flg=1;
					header("location:cnfrmcis.php?flg=$flg&year=$year1&section=$section1&subject=$subject1&fac_id=$fac_id&per_num=$period&dis=$det1&dis_fac=$this_faculty");
				}
			}
			elseif($errflag)
			{
		...
		{
			header("location:sel_branch_assign.php?thisfac_id=$this_faculty&per_id=$period");
		}
