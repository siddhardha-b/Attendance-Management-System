...
<?php
echo "<h2><center>Logged in as ".$_SESSION['NAME']."</center></h2>";
if(isset($_POST['Reset_Password']))
{
	...
	$pwd_err_flag=false;
	$pwd_err_msg=array();
	$oldpwd=$_POST['cpassword'];
	$newpwd=$_POST['newpwd'];
	$rnewpwd=$_POST['rnewpwd'];
	$user_name=$_SESSION['SESS_FIRST_NAME'];
	$query1="SELECT * FROM faculty_login WHERE user_name=$user_name LIMIT 1";
	...
	if(empty($oldpwd)||empty($newpwd)||empty($rnewpwd))
	{
		...
		{
			$err_flag=true;
			$err_msg[]="Please repeat new password";
		}
	}
	if(empty($oldpwd)&&empty($newpwd)&&empty($rnewpwd))
	{
		$err_flag=true;
		$err_msg[]="All fields are compulsory";
	}
	if(!$err_flag)
	{
		if(!($dbpassword==ABEncryption($user_name,$oldpwd)))
		{
			$pwd_err_flag=true;
			$pwd_err_msg[]="Wrong old Password";
		}
		if(!($newpwd==$rnewpwd))
		{
			$pwd_err_flag=true;
			$pwd_err_msg[]="Passwords does not match";
		}
		if(strlen($newpwd)<6)
		{
			$pwd_err_flag=true;
			$pwd_err_msg[]="Length of password must be atleast 6";
		}
		if($pwd_err_flag)
		{
			foreach($pwd_err_msg as $msg)
			{
				echo "<center>".$msg."<br></center>";
			}
		}
		else
		{
			$password=ABEncryption($user_name,$newpwd);
			$querypwd="UPDATE faculty_login SET password='$password' WHERE user_name='$user_name'";
			$exec=mysql_query($querypwd);
			if(!$exec)
			{
				echo "<center>An unknown error encountered.Please try later</center>";
			}
			else
			{
				echo "<script type=\"text/javascript\">";
				echo "alert(\"Sucessfully changed password\")";
				echo "</script>";
				echo "<h3><center>Sucessfully changed password</center></h3>";
			}
		}
...
<center><form id="form1" name="form1" method="post" action="">
  <table width="500" border="0">
    ...
    <tr>
      ...
      <td><label for="rnewpwd"></label>
      <input type="password" name="rnewpwd" id="rnewpwd" /></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td><input type="submit" name="Reset_Password" id="Reset_Password" value="Reset Password" /></td>
    </tr>
  </table>
</form></center>
