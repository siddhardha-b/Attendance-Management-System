...
if(isset($_POST['nonele']))
{
	...
	{
		$err_flag=true;
		$err_msg[]="Subject Code cannot be empty";
	}
	if(empty($subname))
	{
		$err_flag=true;
		$err_msg[]="Subject Name cannot be empty";

	}
	if(empty($subsfrm))
	{
		$err_flag=true;
		$err_msg[]="Subject Short Notation cannot be empty";
	...
	{
		$querysub="INSERT INTO `mvsratt`.`subject_details` (`subject_id`,`subject_sfrm`,`subject_name` ) VALUES ('$subcode','$subsfrm','$subname');";
		$result=mysql_query($querysub);
		if(!$result)
		{
			echo "An unknown error encontered while entering details.Try later";
		}
		else
		{
			echo "<h2><center>Succesfully added details<center></h2>";
		}
...
if(isset($_POST['ele']))
{
	...
	{
		$querysub="INSERT INTO `mvsratt`.`subject_details` (`subject_id`,`subject_sfrm`,`subject_name` ) VALUES ('$subcode','$subsfrm','$subname');";
		$queryele="INSERT INTO `mvsratt`.`sub_electives` (`ele_id`) VALUES ('$subcode');";
		$result=mysql_query($querysub);
		$result1=mysql_query($queryele);
		if(!$result||!$result1)
		{
			echo "<center>An unknown error encontered while entering details.Try later</center>";
		}
		else
		{
			echo "<center>Succesfully added details</center>";
		}
	}
}
?>
<form id="form1" name="form1" method="post" action="">
  <table width="500" border="0" class="inner_table1">
    <tr>
      <td width="241">Enter Subject Code</td>
      <td width="249"><label for="sc"></label>
      <input type="text" name="sc" id="sc" /></td>
    </tr>
    <tr>
      <td>Enter Subject Name</td>
      <td width="300"><label for="sn"></label>
      <textarea name="sn" id="sn"></textarea></td>
    </tr>		
    <tr>
      <td>Enter Subject Short Notation</td>
      <td><label for="ssn"></label>
      <input name="ssn" type="text" id="ssn" /></td>
    </tr>
    
    <tr>
    <td>&nbsp;</td>
      <td>
      <input type="submit" name="nonele" id="nonele" value="Add subject" />
      <input type="submit" name="ele" id="ele" value="Add as elective subject" />
      </td>
    </tr>
  </table>
</form>
